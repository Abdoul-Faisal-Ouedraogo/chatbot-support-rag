...
import os
from dotenv import load_dotenv # 1. On importe l'outil
load_dotenv()                 # 2. On charge le fichier .env IMMEDIATEMENT
import pickle
import faiss
import numpy as np
import logging
...
class VectorStoreManager:
    ...
    def __init__(self):
        ...
        print(f"DEBUG: Ma clé commence par : {str(MISTRAL_API_KEY)[:5]}...")
    ...
    def _generate_embeddings(self, chunks: List[Dict[str, any]]) -> Optional[np.ndarray]:
        ...
        for i in range(0, len(chunks), EMBEDDING_BATCH_SIZE):
            ...
            #    logging.error(f"  Détails: Status Code={e.status_code}, Message={e.message}")
                ...
                # Le SDK Mistral moderne n'utilise plus e.status_code directement
                logging.error(f"Erreur API Mistral : {str(e)}")
                raise # Force l'arrêt pour ne pas créer un index vide
            except Exception as e:
